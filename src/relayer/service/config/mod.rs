...
use crate::collation::{FstRequestConverter, RequestDispatcher};
...
impl Configuration {
    pub fn recover_keypair(
        ...
        let secret = {
            let crypto = {
                let key_file_content = read_file(keyfile_path)?;
    ...
    pub fn interface(interface: &str) -> IpAddr {
        match interface {
...
fn resolve_path(path_str: &str) -> Option<std::path::PathBuf> {

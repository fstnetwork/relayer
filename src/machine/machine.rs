...
use crate::collation::{ClosedCollation, OpenCollation, RequestConverter, RequestDispatcher};
use crate::ethereum::{
    monitor::Error as EthereumMonitorError, service::Error as EthereumServiceError,
};
use crate::pricer::Error as PriceServiceError;
use crate::traits::{
    self, EthereumMonitor, EthereumMonitorResponse, EthereumMonitorTask, EthereumService,
    PoolRequestTag, PoolService, PriceService,
};
    ...
    C: RequestConverter,
{
    ...
    pub fn adjust_gas_amount(gas_amount: U256, max_gas_limit: U256) -> U256 {
        ...
        let adjusted = U256::min(
            gas_amount + (gas_amount / U256::from(10)) + U256::from(50_000),
            max_gas_limit,
        );
        info!(target: "relayer",
    ...
    fn poll_broadcasting(&mut self) -> Poll<Option<RelayerState>, Error> {
        ...
        match self.ethereum_monitor.lock().subscribe(
